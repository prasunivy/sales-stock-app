...
def _mobile_table(df, compact_cols, detail_title_col, uid_prefix='tbl'):
    ...
    rj = json.dumps(rows)
    cj = json.dumps(compact_cols)
    aj = json.dumps(all_cols)
    dtc = detail_title_col
    css = (
        '<style>'
        '#{t} .ivy-desk {{width:100%;border-collapse:collapse;font-size:.83rem;border:1px solid #e2ece9;overflow:hidden;}}'
        '#{t} .ivy-desk th {{background:#1a6b5a;color:white;font-weight:600;font-size:.78rem;letter-spacing:.04em;text-transform:uppercase;padding:.65rem 1rem;text-align:left;}}'
        '#{t} .ivy-desk td {{padding:.5rem 1rem;border-bottom:1px solid #e2ece9;color:#1c2b27;font-size:.83rem;white-space:nowrap;}}'
        '#{t} .ivy-desk tr:nth-child(even) td {{background:#f0faf7;}}'
        '#{t} .ivy-desk tr:hover td {{background:#e8f5f1;cursor:pointer;}}'
        '#{t} .ivy-mob {{display:none;}}'
        '#{t} .ivy-ctbl {{width:100%;border-collapse:collapse;table-layout:fixed;}}'
        '#{t} .ivy-ctbl th {{background:#1a6b5a;color:white;font-size:.7rem;font-weight:600;padding:5px 4px;text-transform:uppercase;overflow:hidden;text-overflow:ellipsis;white-space:nowrap;}}'
        '#{t} .ivy-ctbl td {{padding:5px 4px;border-bottom:1px solid #e2ece9;overflow:hidden;text-overflow:ellipsis;white-space:nowrap;color:#1c2b27;font-size:.78rem;}}'
        '#{t} .ivy-ctbl tr:nth-child(even) td {{background:#f0faf7;}}'
        '#{t} .ivy-ctbl tr {{cursor:pointer;}}'
        '#{t} .ivy-ctbl tr:active td {{background:#e8f5f1;}}'
        '#{t} .ivy-hint {{text-align:center;font-size:.7rem;color:#9ab4ad;padding:4px 0 2px;}}'
        '#{t} .ivy-tip {{color:#9ab4ad;font-size:.7rem;}}'
        '#{t} .ivy-dtl {{display:none;}}'
        '#{t} .ivy-bk {{display:flex;align-items:center;gap:6px;background:#1a6b5a;color:white;border:none;padding:8px 12px;font-size:.8rem;font-weight:600;cursor:pointer;width:100%;}}'
        '#{t} .ivy-db {{padding:10px 10px 16px;background:#f7f9f8;}}'
        '#{t} .ivy-dt {{font-size:.85rem;font-weight:700;color:#1c2b27;margin-bottom:8px;word-break:break-word;}}'
        '#{t} .ivy-dr {{display:flex;justify-content:space-between;align-items:flex-start;padding:5px 0;border-bottom:1px solid #e2ece9;}}'
        '#{t} .ivy-dl {{font-size:.7rem;color:#5a7268;font-weight:600;text-transform:uppercase;letter-spacing:.04em;min-width:40%;padding-right:8px;}}'
        '#{t} .ivy-dv {{font-size:.78rem;color:#1c2b27;font-weight:500;text-align:right;word-break:break-word;}}'
        '@media (max-width:768px) {{#{t} .ivy-desk {{display:none;}} #{t} .ivy-mob {{display:block;}} }}'
        '</style>'
    ).replace('{t}', table_id)
    body = (
        '<div id="{t}">'
        '<div class="ivy-desk"><table><thead><tr id="{t}_dh"></tr></thead><tbody id="{t}_db"></tbody></table></div>'
        '<div class="ivy-mob">'
        '<div class="ivy-list" id="{t}_ls"><div class="ivy-hint">Tap any row for full details</div>'
        '<table class="ivy-ctbl"><thead><tr id="{t}_mh"></tr></thead><tbody id="{t}_mb"></tbody></table></div>'
        '<div class="ivy-dtl" id="{t}_dtl">'
        '<button class="ivy-bk" onclick="ivy_bk_{t}()">&#8592; Back</button>'
        '<div class="ivy-db"><div class="ivy-dt" id="{t}_dtt"></div><div id="{t}_dr"></div></div>'
        '</div></div></div>'
    ).replace('{t}', table_id)
    js = (
        '<script>(function(){'
        'var R={rj},C={cj},A={aj},T="{t}",D="{d}";'
        'var dh=document.getElementById(T+"_dh");'
        'A.forEach(function(c){var e=document.createElement("th");e.textContent=c;dh.appendChild(e);});'
        'var db=document.getElementById(T+"_db");'
        'R.forEach(function(r,i){var tr=document.createElement("tr");tr.style.cursor="pointer";'
        'A.forEach(function(c){var td=document.createElement("td");td.textContent=r[c]||"";tr.appendChild(td);});'
        'tr.onclick=function(){W["ivs_"+T](i);};db.appendChild(tr);});'
        'var mh=document.getElementById(T+"_mh");'
        'C.forEach(function(c){var e=document.createElement("th");e.textContent=c;mh.appendChild(e);});'
        'var te=document.createElement("th");te.style.width="14px";mh.appendChild(te);'
        'var mb=document.getElementById(T+"_mb");'
        'R.forEach(function(r,i){var tr=document.createElement("tr");'
        'C.forEach(function(c){var td=document.createElement("td");td.textContent=r[c]||"";tr.appendChild(td);});'
        'var ti=document.createElement("td");ti.innerHTML="<span class=\\"ivy-tip\\">&#8250;</span>";tr.appendChild(ti);'
        'tr.onclick=function(){W["ivs_"+T](i);};mb.appendChild(tr);});'
        'var W=window;'
        'W["ivs_"+T]=function(i){var r=R[i];'
        'document.getElementById(T+"_dtt").textContent=r[D]||("Row "+(i+1));'
        'var dr=document.getElementById(T+"_dr");dr.innerHTML="";'
        'A.forEach(function(c){if(r[c]===""||r[c]==null)return;'
        'var d=document.createElement("div");d.className="ivy-dr";'
        'd.innerHTML="<span class=\\"ivy-dl\\">"+ c +"</span><span class=\\"ivy-dv\\">"+(r[c]||"\u2014")+"</span>";'
        'dr.appendChild(d);});'
        'document.getElementById(T+"_ls").style.display="none";'
        'document.getElementById(T+"_dtl").style.display="block";};'
        'W["ivy_bk_"+T]=function(){document.getElementById(T+"_dtl").style.display="none";document.getElementById(T+"_ls").style.display="block";};'
        '})();</script>'
    ).replace('{rj}', rj).replace('{cj}', cj).replace('{aj}', aj).replace('{t}', table_id).replace('{d}', dtc)
...
""")
    ...
    **Date:** {invoice['ops_date']}  
    ...
    # =========================
        ...
        if payment_mode == "Cheque":
            ...
            with col1:
                ...
                if cheque_no:
                    payment_details["cheque_number"] = cheque_no
            with col2:
                cheque_date = st.date_input("Cheque Date*", key="cheque_date")
                payment_details["cheque_date"] = cheque_date.isoformat()
            ...
            with col4:
                branch_name = st.text_input("Branch (Optional)", key="branch_name")
                if branch_name:
                    payment_details["branch_name"] = branch_name
            
        elif payment_mode in ["UPI", "NEFT/RTGS", "Bank Transfer", "IMPS"]:
            st.subheader("Transaction Details")
            col1, col2 = st.columns(2)
            with col1:
                transaction_id = st.text_input("Transaction ID*", key="txn_id")
                if transaction_id:
                    payment_details["transaction_id"] = transaction_id
            with col2:
                utr_no = st.text_input("UTR/Reference Number", key="utr_no")
                if utr_no:
                    payment_details["utr_number"] = utr_no
            
            if payment_mode in ["NEFT/RTGS", "Bank Transfer", "IMPS"]:
                bank_name = st.text_input("Bank Name", key="bank_name_online")
                if bank_name:
                    payment_details["bank_name"] = bank_name
            ...
            try:
                ...
                for invoice_id, amt in allocs.items():
                    if amt > 0:
                        ...
                        invoice = admin_supabase.table("ops_documents")\
                            .select("invoice_total, paid_amount, outstanding_balance")\
                            .eq("id", invoice_id)\
                            .single()\
                            .execute().data
        
                        if invoice:
                            new_paid = (invoice.get("paid_amount") or 0) + amt
    ...
    # =========================
        ...
        _mobile_table(
            df,
            compact_cols=["Date", "Invoice No", "Type", "Balance Due"],
            detail_title_col="Invoice No",
            uid_prefix="fin_ledger"
        )
    ...
    # =========================
        ...
        _mobile_table(
            df_stock,
            compact_cols=["Date", "Voucher No", "In Qty", "Out Qty", "Closing Qty"],
            detail_title_col="Voucher No",
            uid_prefix="stock_ledger"
        )
    ...
    # =========================
        ...
        inv_search = st.text_input(
            "🔍 Search by OPS No, Reference No, or Party Name",
            placeholder="e.g. OPS-2024, INV-001, Sharma Medicals...",
            key="inv_search"
        ).strip().lower()
        ...
        if inv_search:
            def _inv_match(inv):
                if inv_search in (inv.get("ops_no") or "").lower(): return True
                if inv_search in (inv.get("reference_no") or "").lower(): return True
                # Match party name from masters
                for eid in [inv.get("from_entity_id"), inv.get("to_entity_id")]:
                    if not eid: continue
                    s = next((x for x in st.session_state.stockists_master if x["id"] == eid), None)
                    if s and inv_search in s["name"].lower(): return True
                    u = next((x for x in st.session_state.users_master if x["id"] == eid), None)
                    if u and inv_search in u["username"].lower(): return True
                    c = next((x for x in st.session_state.cnfs_master if x["id"] == eid), None)
                    if c and inv_search in c["name"].lower(): return True
                return False
            invoices = [i for i in invoices if _inv_match(i)]
            st.caption(f"📋 {len(invoices)} result(s) for \"{inv_search}\"")
            if not invoices:
    ...
    **Archived Date:** {invoice.get('updated_at', 'N/A')}
    ...
    # =========================
        ...
        cn_search = st.text_input(
            "🔍 Search by OPS No, Reference No, or Party Name",
            placeholder="e.g. CRN-001, Sharma Medicals...",
            key="cn_search"
        ).strip().lower()
        ...
        if cn_search:
            def _cn_match(d):
                if cn_search in (d.get("ops_no") or "").lower(): return True
                if cn_search in (d.get("reference_no") or "").lower(): return True
                for eid in [d.get("from_entity_id"), d.get("to_entity_id")]:
                    if not eid: continue
                    s = next((x for x in st.session_state.stockists_master if x["id"] == eid), None)
                    if s and cn_search in s["name"].lower(): return True
                    u = next((x for x in st.session_state.users_master if x["id"] == eid), None)
                    if u and cn_search in u["username"].lower(): return True
                return False
            docs = [d for d in docs if _cn_match(d)]
            st.caption(f"📋 {len(docs)} result(s) for \"{cn_search}\"")
            if not docs:
                st.warning("No credit notes match your search.")
                st.stop()
        ...
        for entry in display_data:
            with st.container():
                ...
                with col4:
                    ...
                    with b1:
                        if st.button("👁", key=f"view_{entry['id']}"):
            ...
            # CRITICAL: Expanders OUTSIDE the columns, OUTSIDE the st.container()
            # This makes them full width!
            
            # VIEW SECTION - FULL WIDTH
            if st.session_state.get(f"show_view_{entry['id']}", False):
                with st.expander("📋 Transaction Details", expanded=True):
                    ...
                    with info_col3:
                        st.metric("TO", entry['to_entity'])
                        st.metric("Items", f"R:{entry['return_products']}" + (f" | P:{entry['replace_products']}" if entry['replace_products'] > 0 else ""))
                    ...
                    if entry['stock_moves']:
                        ...
                        for sm in entry['stock_moves']:
                            ...
                            if "return" in sm["narration"].lower():
                                if sm["direction"] == "OUT":
                                    return_items.append(f"➖ {product_name}: {sm['qty_out']} (from {sm['entity_type']})")
                                else:
                                    return_items.append(f"➕ {product_name}: {sm['qty_in']} (to {sm['entity_type']})")
                            elif "replacement" in sm["narration"].lower():
                                if sm["direction"] == "OUT":
                                    replace_items.append(f"➖ {product_name}: {sm['qty_out']} (from {sm['entity_type']})")
                                else:
                                    replace_items.append(f"➕ {product_name}: {sm['qty_in']} (to {sm['entity_type']})")
                    ...
                    if entry['net_amount'] > 0:
                        st.success(f"Debited ₹{abs(entry['net_amount']):,.2f}")
                    elif entry['net_amount'] < 0:
                        st.info(f"Credited ₹{abs(entry['net_amount']):,.2f}")
                    else:
                        st.info("No financial impact")
                    
                    if st.button("✖️ Close", key=f"close_view_{entry['id']}"):
                        st.session_state[f"show_view_{entry['id']}"] = False
                        st.rerun()
            
            # EDIT SECTION - FULL WIDTH
            if st.session_state.get(f"show_edit_{entry['id']}", False):
                with st.expander("⚠️ How to Edit", expanded=True):
                    st.info("**Steps:** 1) Copy details below 2) Delete this transaction 3) Create new with correct details")
                    
                    copy_col1, copy_col2 = st.columns(2)
                    
                    with copy_col1:
                        st.text(f"""Doc: {entry['ops_no']}
Date: {entry['ops_date']}
Type: {entry['txn_type']}
FROM: {entry['from_entity']}""")
                    
                    with copy_col2:
                        st.text(f"""TO: {entry['to_entity']}
Return: {entry['return_products']}
Replace: {entry['replace_products']}
Amount: ₹{abs(entry['net_amount']):,.2f}""")
                    
                    if st.button("✖️ Close", key=f"close_edit_{entry['id']}"):
                        st.session_state[f"show_edit_{entry['id']}"] = False
                        st.rerun()
            
            # DELETE SECTION - FULL WIDTH
            if st.session_state.get(f"show_delete_{entry['id']}", False):
                        ...
                        try:
                            ...
                            for sm in entry['stock_moves']:
                                admin_supabase.table("stock_ledger").insert({
                                    ...
                                    "closing_qty": 0,
                                    "direction": "ADJUST",
                                    "narration": f"Reversal of {entry['ops_no']}"
                                }).execute()
                            ...
                            admin_supabase.table("audit_logs").insert({
                                ...
                                "target_id": entry["id"],
                                "performed_by": user_id,
                                "message": f"{entry['ops_no']} deleted - reversed"
                            }).execute()
                            
                            st.session_state[f"show_delete_{entry['id']}"] = False
        ...
        if st.button("📥 Export to Excel"):
            ...
            with pd.ExcelWriter(output, engine='openpyxl') as writer:
                df.to_excel(writer, index=False, sheet_name="Return Replace")
            
            st.download_button(
                "Download Excel",
                output.getvalue(),
                f"return_replace_{from_date}_to_{to_date}.xlsx",
    ...
    # =========================
        ...
        with cn_freight_tab:
            st.info("""
            📌 **Apply Credit Notes and Freight charges directly to invoices — no payment needed.**
            - Select a stockist to see their CNs and freight credits
            - Already-allocated ones are shown greyed out for reference
            - Choose which invoice each available CN/freight applies to, then Confirm
            """)

            # ── Select Stockist ───────────────────────────────────────
            all_stockists_cnf = sorted(
                st.session_state.stockists_master, key=lambda x: x["name"]
            )
            stockist_options_cnf = {s["name"]: s["id"] for s in all_stockists_cnf}
            sel_sname_cnf = st.selectbox(
                "Select Stockist",
                list(stockist_options_cnf.keys()),
                key="cnf_stockist_select"
            )
            sel_sid_cnf = stockist_options_cnf[sel_sname_cnf]

            st.divider()

            # ── Fetch outstanding invoices ────────────────────────────
            cnf_invoices = admin_supabase.table("ops_documents")                .select("id, ops_no, ops_date, invoice_total, outstanding_balance")                .eq("ops_type", "STOCK_OUT")                .eq("stock_as", "normal")                .eq("to_entity_id", sel_sid_cnf)                .gt("outstanding_balance", 0)                .eq("is_deleted", False)                .order("ops_date")                .execute().data or []

            inv_opts_cnf = {"— Don't apply —": None}
            for inv in cnf_invoices:
                inv_opts_cnf[f"{inv['ops_no']} (Due: ₹{float(inv['outstanding_balance']):,.2f})"] = inv["id"]

            # ================================================================
            # CREDIT NOTES — Comprehensive fetch
            # Method A: ops_documents by from/to entity
            # Method B: financial_ledger by party_id
            # Merge both to catch all CNs regardless of how they were stored
            # ================================================================
            cn_docs_a = admin_supabase.table("ops_documents")                .select("id, ops_no, ops_date, invoice_total")                .eq("stock_as", "credit_note")                .eq("is_deleted", False)                .eq("from_entity_id", sel_sid_cnf)                .execute().data or []

            cn_docs_b = admin_supabase.table("ops_documents")                .select("id, ops_no, ops_date, invoice_total")                .eq("stock_as", "credit_note")                .eq("is_deleted", False)                .eq("to_entity_id", sel_sid_cnf)                .execute().data or []

            # Method C: via financial_ledger.party_id (first row per doc only)
            fl_rows = admin_supabase.table("financial_ledger")                .select("ops_document_id, credit")                .eq("party_id", sel_sid_cnf)                .gt("credit", 0)                .execute().data or []

            # First credit per document only (avoid summing product lines)
            fl_first_credit = {}
            fl_credit_doc_ids = set()
            for r in fl_rows:
                oid = r.get("ops_document_id")
                if oid and oid not in fl_first_credit:
                    fl_first_credit[oid] = float(r.get("credit") or 0)
                    fl_credit_doc_ids.add(oid)

            cn_docs_c = []
            if fl_credit_doc_ids:
                cn_docs_c = admin_supabase.table("ops_documents")                    .select("id, ops_no, ops_date, invoice_total")                    .eq("stock_as", "credit_note")                    .eq("is_deleted", False)                    .in_("id", list(fl_credit_doc_ids))                    .execute().data or []

            # Merge all three, deduplicate
            seen_cn = set()
            all_cn_docs = []
            for doc in cn_docs_a + cn_docs_b + cn_docs_c:
                if doc["id"] not in seen_cn:
                    seen_cn.add(doc["id"])
                    all_cn_docs.append(doc)
            all_cn_docs.sort(key=lambda x: x["ops_date"])

            # Get CN amounts — prefer invoice_total from ops_documents
            # (more reliable than summing financial_ledger lines)
            cn_total_map = {}
            for doc in all_cn_docs:
                doc_id   = doc["id"]
                inv_tot  = float(doc.get("invoice_total") or 0)
                if inv_tot > 0:
                    cn_total_map[doc_id] = inv_tot
                elif doc_id in fl_first_credit:
                    cn_total_map[doc_id] = fl_first_credit[doc_id]
                else:
                    # Last resort: fetch first ledger row for this doc
                    led = admin_supabase.table("financial_ledger")                        .select("credit")                        .eq("ops_document_id", doc_id)                        .gt("credit", 0)                        .limit(1)                        .execute().data or []
                    cn_total_map[doc_id] = float(led[0]["credit"]) if led else 0.0

            # Get already applied amounts from payment_settlements
            cn_applied_map = {}
            for doc in all_cn_docs:
                setts = admin_supabase.table("payment_settlements")                    .select("amount")                    .eq("payment_ops_id", doc["id"])                    .execute().data or []
                cn_applied_map[doc["id"]] = sum(float(s["amount"]) for s in setts)

            # Build full CN list with status
            cn_full_list = []
            for doc in all_cn_docs:
                doc_id    = doc["id"]
                total     = cn_total_map.get(doc_id, 0.0)
                applied   = cn_applied_map.get(doc_id, 0.0)
                ...
                if available <= 0:
                    ...
                    status = "🔴 Unallocated"
                cn_full_list.append({
                    "id": doc_id, "ops_no": doc["ops_no"],
                    "ops_date": doc["ops_date"],
                    "total": total, "applied": applied,
                    "available": available, "status": status
                })

            avail_cns = [c for c in cn_full_list if c["available"] > 0]

            # ================================================================
            # FREIGHT — via financial_ledger.party_id + ops_no filter
            # ================================================================
            freight_items = []
            for oid, credit_amt in fl_first_credit.items():
                # Skip docs already identified as CNs
                if oid in seen_cn:
                    continue
                if credit_amt <= 0:
                    continue
                # Check if it's a freight document
                fr_doc = admin_supabase.table("ops_documents")                    .select("id, ops_no, ops_date, narration, is_deleted")                    .eq("id", oid)                    .execute().data or []
                if not fr_doc:
                    continue
                fr_doc = fr_doc[0]
                if fr_doc.get("is_deleted"):
                    continue
                ops_no  = (fr_doc.get("ops_no") or "").upper()
                narr    = (fr_doc.get("narration") or "").lower()
                if "freight" not in ops_no and "freight" not in narr:
                    continue
                # Check applied
                setts = admin_supabase.table("payment_settlements")                    .select("amount")                    .eq("payment_ops_id", oid)                    .execute().data or []
                fr_applied  = sum(float(s["amount"]) for s in setts)
                fr_available = max(0.0, credit_amt - fr_applied)
                freight_items.append({
                    "id": oid,
                    "ops_no":    fr_doc.get("ops_no", "—"),
                    "ops_date":  fr_doc.get("ops_date", "—"),
                    "narration": fr_doc.get("narration", ""),
                    "total":     credit_amt,
                    "applied":   fr_applied,
                    "available": fr_available,
                })

            avail_freight = [f for f in freight_items if f["available"] > 0]

            # ================================================================
            # RENDER
            # ================================================================
            cnf_allocations = {}  # {doc_id: (inv_id, amount)}

            # ── Credit Notes ──────────────────────────────────────────
            if cn_full_list:
                st.markdown("### 📝 Credit Notes")
                for cn in cn_full_list:
                    is_available = cn["available"] > 0
                    with st.container():
                        c1, c2, c3, c4, c5 = st.columns([2, 1, 1, 2, 2])
                        with c1:
                            label = cn["ops_no"] if is_available else f"~~{cn['ops_no']}~~"
                            st.write(f"**{cn['ops_no']}**")
                            st.caption(f"📅 {cn['ops_date']}")
                        with c2:
                            st.write("**Total:**")
                            st.write(f"₹{cn['total']:,.2f}")
                        with c3:
                            st.write("**Available:**")
                            color = "green" if is_available else "grey"
                            st.markdown(
                                f"<span style='color:{color};font-weight:700'>₹{cn['available']:,.2f}</span>",
                                unsafe_allow_html=True
                            )
                        with c4:
                            st.caption(cn["status"])
                            if cn["applied"] > 0:
                                st.caption(f"Applied: ₹{cn['applied']:,.2f}")
                        with c5:
                            if is_available:
                                sel_inv_label = st.selectbox(
                                    "Apply to Invoice",
                                    list(inv_opts_cnf.keys()),
                                    key=f"cnf_cn_inv_{cn['id']}"
                                )
                                sel_inv_id = inv_opts_cnf[sel_inv_label]
                                if sel_inv_id:
                                    inv_due = next(
                                        (float(i["outstanding_balance"]) for i in cnf_invoices if i["id"] == sel_inv_id), 0.0
                                    )
                                    max_amt = min(cn["available"], inv_due)
                                    amt = st.number_input(
                                        "Amount ₹", min_value=0.0,
                                        max_value=float(max_amt),
                                        value=float(max_amt),
                                        step=0.01,
                                        key=f"cnf_cn_amt_{cn['id']}"
                                    )
                                    if amt > 0:
                                        cnf_allocations[cn["id"]] = (sel_inv_id, amt)
                            else:
                                st.caption("—")
                        st.divider()
            elif not all_cn_docs:
                st.info("No credit notes found for this stockist.")

            # ── Freight ───────────────────────────────────────────────
            if freight_items:
                st.markdown("### 🚚 Freight Credits")
                for fr in freight_items:
                    is_available = fr["available"] > 0
                    with st.container():
                        c1, c2, c3, c4 = st.columns([2, 1, 2, 2])
                        with c1:
                            st.write(f"**{fr['ops_no']}**")
                            st.caption(f"📅 {fr['ops_date']}")
                            if fr["narration"]:
                        ...
                        with c3:
                            if is_available:
                                sel_inv_label = st.selectbox(
                                    "Apply to Invoice",
                                    list(inv_opts_cnf.keys()),
                                    key=f"cnf_fr_inv_{fr['id']}"
                                )
                                sel_inv_id = inv_opts_cnf[sel_inv_label]
                            else:
                                st.caption("✅ Fully Allocated")
                                sel_inv_id = None
                        with c4:
                            if is_available and sel_inv_id:
                                inv_due = next(
                                    (float(i["outstanding_balance"]) for i in cnf_invoices if i["id"] == sel_inv_id), 0.0
                                )
                                max_amt = min(fr["available"], inv_due)
                                amt = st.number_input(
                                    "Amount ₹", min_value=0.0,
                                    max_value=float(max_amt),
                                    value=float(max_amt),
                                    step=0.01,
                                    key=f"cnf_fr_amt_{fr['id']}"
                                )
                                if amt > 0:
                                    cnf_allocations[fr["id"]] = (sel_inv_id, amt)
                            else:
                                st.write("—")
                        st.divider()
            ...
            # ── Confirm ───────────────────────────────────────────────
            if cnf_allocations:
                total_cnf = sum(v[1] for v in cnf_allocations.values())
                st.metric("Total to Apply", f"₹{total_cnf:,.2f}")

                if st.button("✅ Confirm — Apply to Invoices", type="primary", key="cnf_confirm"):
                    try:
                        uid = resolve_user_id()
                        for doc_id, (inv_id, amt) in cnf_allocations.items():
                            # CN/freight total
                            all_totals  = cn_total_map if doc_id in cn_total_map else {}
                            doc_total   = cn_total_map.get(doc_id) or fl_first_credit.get(doc_id, 0.0)

                            # Already applied before this action
                            ...
                            if inv:
                                ...
                                new_out  = float(inv.get("invoice_total") or 0) - new_paid
                                new_st   = "PAID" if new_out <= 0 else ("PARTIAL" if new_paid > 0 else "UNPAID")
                                admin_supabase.table("ops_documents").update({
                                    "paid_amount": new_paid,
                            ...
                            admin_supabase.table("audit_logs").insert({
                                ...
                                "performed_by": uid,
                                "message": (
                                    f"CN/Freight applied ₹{amt:,.2f} to invoice. "
                                    f"Status: {alloc_status}. "
                                    f"Total: ₹{doc_total:,.2f}, Applied so far: ₹{total_applied_now:,.2f}"
                                )
                            }).execute()

                        st.success(f"✅ Applied {len(cnf_allocations)} item(s) successfully.")
                        st.rerun()
                    except Exception as e:
                        st.error(f"❌ Error: {e}")
            elif not avail_cns and not avail_freight:
                if cn_full_list:
                    st.success("✅ All credit notes for this stockist are already fully allocated.")
                elif not all_cn_docs and not freight_items:
                    st.info("No credit notes or freight found for this stockist.")
        ...
        with reverse_tab:
            st.info("""
            🛠️ **How to fix a wrong allocation or wrong party:**

            **Wrong invoice allocated (e.g. ₹30,000 went to Gupta instead of Sharma):**
            - **Step 1:** Select the payment below
            - **Step 2:** Find the wrong line and click 🔄 Reverse
            - **Step 3:** Invoice balance is restored and payment amount is freed
            - **Step 4:** Go to Allocate tab and re-allocate correctly

            **Whole payment under wrong party:**
            - **Step 1:** Select the payment, reverse ALL allocation lines
            - **Step 2:** Go to Payment Register → Edit Party
            - **Step 3:** Come back here and allocate correctly
            """)

            rev_search = st.text_input(
                "🔍 Search by Payment No or Party Name",
                placeholder="e.g. PAY-2024, Sharma...",
                key="rev_search"
            ).strip().lower()

            rev_payments_raw = admin_supabase.table("ops_documents")\
                .select("id, ops_no, ops_date, from_entity_type, from_entity_id, payment_mode, allocation_status")\
                .eq("ops_type", "ADJUSTMENT")\
                .eq("is_deleted", False)\
                .in_("allocation_status", ["FULLY_ALLOCATED", "PARTIALLY_ALLOCATED"])\
                .order("ops_date", desc=True)\
                .execute().data or []

            if not rev_payments_raw:
                st.info("No allocated payments found. Nothing to reverse.")
            else:
                rev_options = {}
                for p in rev_payments_raw:
                    if p.get("from_entity_type") == "Stockist" and p.get("from_entity_id"):
                        s = next((x for x in st.session_state.stockists_master if x["id"] == p["from_entity_id"]), None)
                        fname = s["name"] if s else "Unknown"
                    elif p.get("from_entity_type") == "User" and p.get("from_entity_id"):
                        u = next((x for x in st.session_state.users_master if x["id"] == p["from_entity_id"]), None)
                        fname = u["username"] if u else "Unknown"
                    elif p.get("from_entity_type") == "CNF" and p.get("from_entity_id"):
                        c = next((x for x in st.session_state.cnfs_master if x["id"] == p["from_entity_id"]), None)
                        fname = c["name"] if c else "Unknown"
                    else:
                        fname = p.get("from_entity_type", "Unknown")
                    # Search filter
                    if rev_search and rev_search not in p["ops_no"].lower() and rev_search not in fname.lower():
                        continue
                    status_icon = "🟢" if p["allocation_status"] == "FULLY_ALLOCATED" else "🟡"
                    label = f"{status_icon} {p['ops_no']} | {p['ops_date']} | {fname} | {p.get('payment_mode') or 'N/A'}"
                    rev_options[label] = p["id"]

                if not rev_options:
                    st.warning("No payments match your search.")
                else:
                    rev_selected_label = st.selectbox(
                        "Select Payment to View/Reverse",
                        list(rev_options.keys()),
                        key="rev_payment_select"
                    )
                    rev_payment_id = rev_options[rev_selected_label]

                    settlements = admin_supabase.table("payment_settlements")\
                        .select("id, invoice_id, amount")\
                        .eq("payment_ops_id", rev_payment_id)\
                        .execute().data or []

                    if not settlements:
                        st.info("No allocation records found for this payment.")
                    else:
                        st.write(f"**{len(settlements)} allocation(s) found:**")
                        st.divider()
                        for sett in settlements:
                            inv_id   = sett["invoice_id"]
                            sett_amt = float(sett["amount"])
                            inv_doc  = admin_supabase.table("ops_documents")\
                                .select("ops_no, ops_date, narration, invoice_total, paid_amount")\
                                .eq("id", inv_id).limit(1).execute().data
                            inv_doc  = inv_doc[0] if inv_doc else {}
                            is_ob    = inv_doc.get("narration") == "Opening Balance"
                            label_type = "💰 Opening Balance" if is_ob else f"📄 {inv_doc.get('ops_no', 'Unknown')}"
                            col_a, col_b, col_c = st.columns([3, 2, 2])
                            with col_a:
                                st.write(f"**{label_type}**")
                                st.caption(f"Date: {inv_doc.get('ops_date', 'N/A')}")
                            with col_b:
                                st.write(f"**Allocated: ₹{sett_amt:,.2f}**")
                            with col_c:
                                if st.button("🔄 Reverse", key=f"rev_sett_{sett['id']}"):
                                    try:
                                        admin_supabase.table("payment_settlements").delete().eq("id", sett["id"]).execute()
                                        if is_ob:
                                            admin_supabase.table("financial_ledger").delete()\
                                                .eq("ops_document_id", inv_id)\
                                                .eq("parent_ops_id", rev_payment_id).execute()
                                        else:
                                            cur = admin_supabase.table("ops_documents")\
                                                .select("paid_amount, invoice_total")\
                                                .eq("id", inv_id).single().execute().data
                                            if cur:
                                                new_paid = max(0, float(cur.get("paid_amount") or 0) - sett_amt)
                                                inv_total = float(cur.get("invoice_total") or 0)
                                                new_out   = inv_total - new_paid
                                                new_st    = "PAID" if new_out <= 0 else ("PARTIAL" if new_paid > 0 else "UNPAID")
                                                admin_supabase.table("ops_documents").update({
                                                    "paid_amount": new_paid,
                                                    "outstanding_balance": max(0, new_out),
                                        ...
                                        rem = admin_supabase.table("payment_settlements")\
                                            .select("amount").eq("payment_ops_id", rev_payment_id).execute().data or []
                                        rem_total = sum(float(s["amount"]) for s in rem)
                                        pay_led   = admin_supabase.table("financial_ledger")\
                                            .select("credit").eq("ops_document_id", rev_payment_id).execute().data or []
                                        pay_total = sum(float(x["credit"]) for x in pay_led)
                                        new_alloc_st = "UNALLOCATED" if rem_total <= 0 else ("FULLY_ALLOCATED" if rem_total >= pay_total else "PARTIALLY_ALLOCATED")
                                        admin_supabase.table("ops_documents").update({"allocation_status": new_alloc_st}).eq("id", rev_payment_id).execute()
                                        admin_supabase.table("audit_logs").insert({
                                            "action": "REVERSE_ALLOCATION",
                                            "target_type": "payment_settlements",
                                            "target_id": rev_payment_id,
                                            "performed_by": resolve_user_id(),
                                            "message": f"Reversed allocation of ₹{sett_amt:,.2f} from {label_type}"
                                        }).execute()
                                        st.success(f"✅ Reversed ₹{sett_amt:,.2f} from {label_type}")
                                        st.rerun()
                                    except Exception as e:
        ...
        # ================================================================
        # TAB 1 — ALLOCATE PAYMENTS
        # ================================================================
        with alloc_tab:
            st.info("""
            📌 **How to allocate a payment to invoices:**
            - **Step 1:** Select the payment from the dropdown below
            - **Step 2:** Enter amounts against Opening Balance (if any) and each invoice
            - **Step 3:** Click Confirm Allocation
            """)
            ...
            for p in filtered_payments:
                ...
                if p["from_entity_type"] == "Company":
                    ...
                    stockist = admin_supabase.table("stockists").select("name").eq("id", p["from_entity_id"]).single().execute()
                    from_name = stockist.data["name"] if stockist.data else "Unknown"
                elif p["from_entity_type"] == "CNF" and p.get("from_entity_id"):
                    cnf = admin_supabase.table("cnfs").select("name").eq("id", p["from_entity_id"]).single().execute()
                    from_name = cnf.data["name"] if cnf.data else "Unknown"
                elif p["from_entity_type"] == "User" and p.get("from_entity_id"):
                    user = admin_supabase.table("users").select("username").eq("id", p["from_entity_id"]).single().execute()
            ...
            **Available to Allocate:** ₹{remaining_to_allocate:,.2f}
            ...
            ob_doc_id     = None
            if ob_doc_ids:
                ob_doc_id = ob_doc_ids[0]
                ob_setts  = admin_supabase.table("payment_settlements")\
                    .select("amount").eq("invoice_id", ob_doc_id).execute().data
                ob_already_paid = sum(float(s["amount"]) for s in ob_setts)
            ...
            # CN can have stockist in from_entity_id OR to_entity_id
            # ========================================================================
            cn_from = admin_supabase.table("ops_documents")            .select("id, ops_no, ops_date")            .eq("stock_as", "credit_note")            .eq("from_entity_id", target_stockist_id)            .eq("is_deleted", False)            .execute().data or []

            cn_to = admin_supabase.table("ops_documents")            .select("id, ops_no, ops_date")            .eq("stock_as", "credit_note")            .eq("to_entity_id", target_stockist_id)            .eq("is_deleted", False)            .execute().data or []

            # Merge and deduplicate
            seen_cn_ids = set()
            cn_docs = []
            for cn in cn_from + cn_to:
                if cn["id"] not in seen_cn_ids:
                    seen_cn_ids.add(cn["id"])
                    cn_docs.append(cn)
            cn_docs.sort(key=lambda x: x["ops_date"])

            # Get CN amounts from financial_ledger (debit OR credit — take whichever has value)
            cn_amount_map = {}  # cn_id → total CN amount
            if cn_docs:
                cn_ids = [r["id"] for r in cn_docs]
                cn_ledger = admin_supabase.table("financial_ledger")                .select("ops_document_id, debit, credit")                .in_("ops_document_id", cn_ids)                .execute().data or []
                ...
                with st.container():
                    ...
                    with col3:
                        ...
                        ob_allocation = st.number_input(
                            "Allocate ₹", min_value=0.0, max_value=float(max_ob),
                            value=0.0, step=0.01, key="alloc_opening_balance"
                        )
                        if ob_allocation > 0:
                ...
                with st.container():
                    ...
                    with col3:
                        ...
                        allocate_amt = st.number_input(
                            "Allocate ₹", min_value=0.0, max_value=max_allocate,
                            value=0.0, step=0.01, key=f"alloc_{inv['id']}"
                        )
                        if allocate_amt > 0:
            ...
            with col2:
                if st.button("✅ Confirm Allocation", type="primary"):
                    ...
                    try:
                        ...
                        # Allocate against Opening Balance
        ...
        pay_search = st.text_input(
            "🔍 Search by Payment No, Reference No, or Party Name",
            placeholder="e.g. PAY-2024, Sharma Medicals, UTR123...",
            key="pay_search"
        ).strip().lower()
        ...
        if pay_search:
            def _pay_match(p):
                if pay_search in (p.get("ops_no") or "").lower(): return True
                if pay_search in (p.get("reference_no") or "").lower(): return True
                if pay_search in (p.get("narration") or "").lower(): return True
                for eid in [p.get("from_entity_id"), p.get("to_entity_id")]:
                    if not eid: continue
                    s = next((x for x in st.session_state.stockists_master if x["id"] == eid), None)
                    if s and pay_search in s["name"].lower(): return True
                    u = next((x for x in st.session_state.users_master if x["id"] == eid), None)
                    if u and pay_search in u["username"].lower(): return True
                    c = next((x for x in st.session_state.cnfs_master if x["id"] == eid), None)
                    if c and pay_search in c["name"].lower(): return True
                return False
            payments = [p for p in payments if _pay_match(p)]
            if not payments:
                st.warning("No payments match your search.")
                st.stop()
...
""")
                ...
                if st.session_state.get("pay_edit_open") and st.session_state.get("pay_edit_id") == payment["id"]:
                    existing_setts = admin_supabase.table("payment_settlements")\
                        .select("id, amount").eq("payment_ops_id", payment["id"]).execute().data or []
                    if existing_setts:
                        total_sett = sum(float(s["amount"]) for s in existing_setts)
                        st.error(f"""
                        ❌ **Cannot edit party — this payment has existing allocations.**

                        ₹{total_sett:,.2f} is already allocated to {len(existing_setts)} invoice(s).

                        **Follow these steps first:**
                        - **Step 1:** Go to 🔗 Allocate Payments menu
                        - **Step 2:** Click the 🔄 Reverse/Fix tab
                        - **Step 3:** Select this payment and reverse ALL allocations
                        - **Step 4:** Come back here and click Edit Party again
                        """)
                        if st.button("❌ Close", key=f"close_edit_pay_{payment['id']}"):
                            st.session_state.pay_edit_open = False
                            st.session_state.pay_edit_id   = None
                            st.rerun()
                    else:
                        st.success("""
                        ✅ No allocations found. You can safely edit the party.

                        **Steps:**
                        - **Step 1:** Select the correct FROM entity type and name below
                        - **Step 2:** Click Save Party Change
                        - **Step 3:** Go to Allocate Payments to re-allocate correctly
                        """)
                        entity_types  = ["Stockist", "User", "CNF", "Company"]
                        cur_type      = payment.get("from_entity_type", "Stockist")
                        cur_type_idx  = entity_types.index(cur_type) if cur_type in entity_types else 0
                        new_from_type = st.selectbox("FROM Entity Type", entity_types,
                            index=cur_type_idx, key=f"new_from_type_{payment['id']}")
                        new_from_id   = None
                        if new_from_type == "Stockist":
                            smap     = {s["name"]: s["id"] for s in st.session_state.stockists_master}
                            cur_name = next((s["name"] for s in st.session_state.stockists_master if s["id"] == payment.get("from_entity_id")), None)
                            cur_idx  = list(smap.keys()).index(cur_name) if cur_name in smap else 0
                            sel = st.selectbox("Select Stockist", list(smap.keys()), index=cur_idx, key=f"new_from_sel_{payment['id']}")
                            new_from_id = smap[sel]
                        elif new_from_type == "User":
                            umap     = {u["username"]: u["id"] for u in st.session_state.users_master}
                            cur_name = next((u["username"] for u in st.session_state.users_master if u["id"] == payment.get("from_entity_id")), None)
                            cur_idx  = list(umap.keys()).index(cur_name) if cur_name in umap else 0
                            sel = st.selectbox("Select User", list(umap.keys()), index=cur_idx, key=f"new_from_sel_{payment['id']}")
                            new_from_id = umap[sel]
                        elif new_from_type == "CNF":
                            cmap     = {c["name"]: c["id"] for c in st.session_state.cnfs_master}
                            cur_name = next((c["name"] for c in st.session_state.cnfs_master if c["id"] == payment.get("from_entity_id")), None)
                            cur_idx  = list(cmap.keys()).index(cur_name) if cur_name in cmap else 0
                            sel = st.selectbox("Select CNF", list(cmap.keys()), index=cur_idx, key=f"new_from_sel_{payment['id']}")
                            new_from_id = cmap[sel]
                        else:
                            st.info("Company selected")
                        col_s, col_c = st.columns(2)
                        with col_s:
                            if st.button("💾 Save Party Change", key=f"save_party_{payment['id']}", type="primary"):
                                try:
                                    admin_supabase.table("ops_documents").update({
                                        "from_entity_type": new_from_type,
                                        "from_entity_id":   new_from_id,
                                        "updated_at":       datetime.utcnow().isoformat()
                                    }).eq("id", payment["id"]).execute()
                                    admin_supabase.table("financial_ledger").update({
                                        "party_id": new_from_id
                                    }).eq("ops_document_id", payment["id"]).execute()
                                    admin_supabase.table("audit_logs").insert({
                                        "action":      "EDIT_PAYMENT_PARTY",
                                        "target_type": "ops_documents",
                                        "target_id":   payment["id"],
                                        "performed_by": resolve_user_id(),
                                        "message":     f"Payment party changed from {payment.get('from_entity_type')} to {new_from_type}"
                                    }).execute()
                                    st.success("✅ Party updated! Go to Allocate Payments to allocate correctly.")
                                    st.session_state.pay_edit_open = False
                                    st.session_state.pay_edit_id   = None
                                    st.rerun()
                                except Exception as e:
                                    st.error(f"❌ Failed: {e}")
                        with col_c:
                            if st.button("❌ Cancel", key=f"cancel_party_{payment['id']}"):
                                st.session_state.pay_edit_open = False
                                st.session_state.pay_edit_id   = None
                                st.rerun()
